...
checkResults (x:xs) =
	...
	else checkResults xs

	
{- checkValidGuess :: [String] -> Bool
checkValidGuess guess = checkResults
    ((map (\x -> getPieceCount guess x <= 1) (oneCountPieces)) ++
    (map (\x -> getPieceCount guess x <= 2) (twoCountPieces)) ++
    (map (\x -> getPieceCount guess x <= 8) (eightCountPieces))) -}
	
{- checkValidGuess :: [String] -> Bool
checkValidGuess guess = checkResults
    ((map (<=1) (getPieceCount guess <$> (oneCountPieces))) ++
     (map (<=2) (getPieceCount guess <$> (twoCountPieces))) ++
     (map (<=8) (getPieceCount guess <$> (eightCountPieces)))) -}
...
maxAllowedCheck guess candidateList maxCount = 
	map (<=maxCount) (getPieceCount guess <$> (candidateList))
